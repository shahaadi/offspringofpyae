...
def propagate_label(node, neighbors, matrix):
    label_weights = dict()
    for n in neighbors:
        if n.label in label_weights:
            label_weights[n.label] += matrix[node.ID][n.ID]
        else:
            label_weights[n.label] = matrix[node.ID][n.ID]
    
    node.label = max(zip(label_weights.values(), label_weights.keys()))[1]
    
def whispers(list_of_nodes, num_times, matrix):
    ...
    for num in num_times:
        ...
        propagate_label(node, neighbors, matrix)
